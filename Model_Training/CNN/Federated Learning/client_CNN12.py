...
config = {"region": value}
wandb.init(project='Cambridge_sales_CNN_FL_Kmean', config=config)
...
class RegressionClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config=None):
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('scatter_plot12.png')
        wandb.log({'Scatter Plot': wandb.Image('scatter_plot12.png')})
        plt.close()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('distribution_plot12.png')
        wandb.log({'Distribution Plot': wandb.Image('distribution_plot12.png')})
        plt.close()
