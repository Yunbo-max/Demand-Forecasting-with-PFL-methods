...
config = {"region": value}
wandb.init(project='Cambridge_sales_CNN_FL_KNN1', config=config)
...
class RegressionClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config=None):
        ...
        wandb.log({'Scatter Plot': wandb.Image('scatter_plot1.png')})
        plt.close()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig('distribution_plot1.png')
        wandb.log({'Distribution Plot': wandb.Image('distribution_plot1.png')})
        plt.close()
