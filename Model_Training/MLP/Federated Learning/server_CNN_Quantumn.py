...
wandb.init(project="SCAIL_server_MLP_sales_FL_new")
...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(
        ...
        """Aggregate fit results using stochastic weighted averaging with respect to participant accuracy."""

        weights_results = [
            (parameters_to_ndarrays(fit_res.parameters), fit_res.num_examples, fit_res.accuracy)
        ...
        weights = [num_examples / total_examples for _, num_examples, _ in weights_results]

        # Calculate participant accuracy
        accuracies = [accuracy for _, _, accuracy in weights_results]

        # Assign higher weights to participants with higher accuracy
        weighted_indices = random.choices(
            range(len(weights_results)),
            weights=accuracies,
            k=len(weights_results)
        )

        # Perform stochastic weighted averaging of the parameters
        aggregated_params = []
        for param_idx in range(len(weights_results[0][0])):
            weighted_param = sum(
                weights[weighted_indices[i]] * weights_results[weighted_indices[i]][0][param_idx]
