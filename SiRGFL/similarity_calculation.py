...
for num_rounds in num_round:
    ...
    if num_rounds ==5:
        ...
        new_std_devs_df = pd.DataFrame(std_devs)   # Create dataframes for variances, means, and standard deviations
                # Concatenate the new row with the existing dataframe
        variances_df = pd.concat([variances_df, new_row_df], ignore_index=True)
        means_df = pd.concat([means_df, new_row_df], ignore_index=True)
        std_devs_df = pd.concat([std_devs_df, new_row_df], ignore_index=True)
